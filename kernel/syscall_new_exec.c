...
fetchint(struct proc *p, uint addr, int *ip)
{
  ...
  if( ((addr >= p->sz)&&(addr < (USERTOP-p->sp_pages*PGSIZE))) || 
      ((addr+4 > p->sz)&&(addr+4 < (USERTOP-p->sp_pages*PGSIZE)))|| 
  ...
  /*
  if ((addr >= proc->code_sz && addr < (proc->code_sz + PGSIZE)) || 
      ((addr+4) > proc->code_sz && (addr+4) < (proc->code_sz + PGSIZE))
      )
  {
    cprintf("int : addr %x from pid %d is illegal. proc code sz = %x  \n", addr, proc->pid, proc->code_sz);
    return -1;
  }
  */
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  /*
  if (addr >= p->code_sz && addr < (p->code_sz + PGSIZE)) {
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if((((uint)i >= proc->sz)&&((uint)i < (USERTOP-proc->sp_pages*PGSIZE))) ||
      (((uint)(i+size) > proc->sz)&&((uint)(i+size) < (USERTOP-proc->sp_pages*PGSIZE))) ||
  ...
  /*
  if (
      ((uint)i >= proc->code_sz && (uint)i < (proc->code_sz + PGSIZE)) || 
      (((uint)i+size) > proc->code_sz && ((uint)i+size) < (proc->code_sz + PGSIZE))
      ) {
